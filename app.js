...
function enterApp(){setPublicScreen('app');$('app-vault-name').textContent=session.vault.name||'Meu Cofre';$('search').value='';$('add-entry').hidden=isExternalKdbx();renderEntries();renderSecurity();renderSettings();navigate('vault');resetIdleTimer();if(isExternalKdbx())showToast('KDBX externo aberto em modo leitura; o original não será regravado.','success');}
...
function openEntryModal(id=null){if(!session)return;if(isExternalKdbx()&&!id)return showToast('KDBX importado é somente leitura no Meu Cofre. Crie/edite itens no KeePassXC.','error');editingId=id;const e=id?session.vault.entries.find(x=>x.id===id):null;const ro=isExternalKdbx();$('entry-modal-title').textContent=ro?'Visualizar item':(e?'Editar item':'Novo item');$('entry-id').value=e?.id||'';$('entry-title').value=e?.title||'';$('entry-username').value=e?.username||'';$('entry-password').value=e?.password||'';$('entry-password').type='password';$('entry-show-password').textContent='Mostrar';$('entry-url').value=e?.url||'';$('entry-tags').value=(e?.tags||[]).join(', ');$('entry-totp').value=e?.totpSecret||'';$('entry-notes').value=e?.notes||'';$('entry-favorite').checked=!!e?.favorite;$('entry-delete').hidden=ro||!e;$('entry-quick-actions').hidden=!e;$('entry-generate-password').hidden=ro;$('entry-form').querySelector('button[type="submit"]').hidden=ro;for(const id2 of ['entry-title','entry-username','entry-password','entry-url','entry-tags','entry-totp','entry-notes'])$(id2).readOnly=ro;$('entry-favorite').disabled=ro;$('entry-modal').hidden=false;updateTotpBox();if(!ro)setTimeout(()=>$('entry-title').focus(),80);}
...
async function persistVault(){if(!session)return;if(isExternalKdbx())throw new Error('KDBX importado está em modo leitura. Edite no KeePassXC e reimporte para preservar grupos, histórico, anexos e campos avançados.');session.vault.updatedAt=new Date().toISOString();if(isLegacySession())record=await legacySaveRecord(record,session.vaultKey,session.vault);else record=await saveStoredKdbx(record,session.vault,session.components,session.publicMeta,session.kdbxInfo?.rounds);await putVaultRecord(record);}
...
async function renderSecurity(){if(!session)return;const legacy=isLegacySession();$('legacy-migration-section').hidden=!legacy;$('kdbx-summary').replaceChildren();$('auth-slots').replaceChildren();$('recovery-key-section').hidden=true;$('change-password-section').hidden=true;
  ...
  await renderDiagnostics();$('backup-status').textContent=isExternalKdbx()?'O KDBX importado é exportado sem alteração. Edite-o no KeePassXC e reimporte após mudanças.':(isKdbxSession()?'O arquivo .kdbx contém o cofre cifrado e autenticado. Exporte após mudanças importantes.':'Migre primeiro para KDBX 4.1.');
}
function renderKdbxSummary(){const info=session.kdbxInfo||{};let kdf=info.kdf||'AES-KDF';if(kdf==='AES-KDF')kdf+=info.rounds?` · ${Number(info.rounds).toLocaleString('pt-BR')} rodadas`:'';else if(/^Argon2/.test(kdf))kdf+=` · ${Number(info.iterations||0).toLocaleString('pt-BR')} iterações · ${Math.round(Number(info.memoryBytes||0)/1048576)} MiB · p=${info.parallelism||1}`;let protection;if(isExternalKdbx()){const p=session.publicMeta.requiresPassword,k=session.publicMeta.requiresKeyFile;protection=p&&k?'Senha + key file':p?'Senha':k?'Key file':'Credenciais KeePassXC';}else protection=modeLabel(session.publicMeta.mode);const data=[['Formato',`KDBX ${info.version||'4.1'}`],['Origem',isExternalKdbx()?'KeePassXC/KeePass importado · byte a byte · leitura':'Meu Cofre'],['Proteção',protection],['Cifrador',info.cipher||'AES-256-CBC'],['KDF',kdf],['Compressão',info.compression||'Nenhuma']];for(const [a,b] of data){const r=document.createElement('div');r.className='row';const m=document.createElement('div');m.className='row-main';const t=document.createElement('div');t.className='row-title';t.textContent=a;const sub=document.createElement('div');sub.className='row-sub';sub.textContent=b;m.append(t,sub);r.append(m);$('kdbx-summary').append(r);}}
