...
let session = null;
let editingId = null;
let selectedGroupUuid = 'all';
let editingGroupUuid = null;
let totpTimer = null;
let idleTimer = null;
...
function bindEvents() {
  ...
  $('group-form').addEventListener('submit', saveGroupFromForm);
  $('group-delete').addEventListener('click', deleteCurrentGroup);
...
function renderUnlockMethods() {
  ...
  $('unlock-password-area').hidden=true; $('unlock-twofactor-area').hidden=true; $('unlock-recovery').hidden=true; $('recovery-password-label').hidden=true;
  if (isLegacyRecord(record)) return renderLegacyUnlock();
  const info=getKdbxHeaderInfo(); const external=Boolean(record?.external); const meta=external?(record.sidecarMeta||{mode:PROTECTION_MODES.EXTERNAL,slots:[]}):(info?.publicMeta||{}); const mode=meta.mode;
  $('unlock-subtitle').textContent=external?`KDBX ${info?.version||''} importado: o arquivo original será preservado byte a byte.`:'O KDBX é autenticado por HMAC antes da descriptografia.';
  if(external){
    const rp=meta.requiresPassword, rk=meta.requiresKeyFile;
    if(rp!==false && rk!==true) $('unlock-password-area').hidden=false;
    if(rk!==false){$('unlock-recovery').hidden=false;$('recovery-password-label').hidden=rp===false;}
    (meta.slots||[]).forEach(slot=>appendKdbxPrfButton(slot,false));
    const req=rp===true&&rk===true?'senha + key file':rp===true?'senha':rk===true?'key file':'senha e/ou key file';
    $('unlock-policy-note').textContent=`KeePassXC/KeePass: use ${req}. AES-KDF, Argon2d/Argon2id, AES-256, ChaCha20 e Twofish são reconhecidos. Key files XML v1/v2, raw, hexadecimal e arquivos arbitrários são aceitos. O Challenge-Response HMAC-SHA1 do KeePassXC é diferente de FIDO2 e continua exigindo KeePassXC/nativo.`;
    return;
  }
  if (!mode || mode===PROTECTION_MODES.PASSWORD) {
    $('unlock-password-area').hidden=false;$('unlock-policy-note').textContent='KDBX 4.1 protegido por senha mestra. YubiKeys cadastradas podem ser usadas como desbloqueio alternativo.';(meta.slots||[]).forEach(slot => appendKdbxPrfButton(slot,false));
  } else if (mode===PROTECTION_MODES.YUBIKEY) {
    $('unlock-policy-note').textContent='KDBX 4.1 protegido por um componente aleatório liberado pela YubiKey. A recuperação usa o arquivo .key.';(meta.slots||[]).forEach(slot => appendKdbxPrfButton(slot,false)); $('unlock-recovery').hidden=false;
  } else {
    $('unlock-policy-note').textContent='KDBX 4.1 com dois componentes: senha mestra + chave liberada pela YubiKey. Ambos são necessários no acesso normal.';$('unlock-twofactor-area').hidden=false; (meta.slots||[]).forEach(slot => appendKdbxPrfButton(slot,true)); $('unlock-recovery').hidden=false; $('recovery-password-label').hidden=false;
  }
}
...
async function unlockWithPassword() {
  ...
  try {
    ...
    else {const u=record?.external?await openStoredKdbxGeneric(record,password,null):await openStoredKdbxWithPassword(record,password);await acceptKdbxOpen(u);}
...
async function unlockKdbxPrf(slot,button,twoFactor) {
  ...
  try{secret=await evaluatePrf(slot);const u=await openStoredKdbxWithPrf(record,slot,secret,password);await acceptKdbxOpen(u);clearSecretInputs();enterApp();}
...
async function unlockWithRecoveryFile(file){
  if(!file||!isKdbxRecord(record))return;let raw=null,key=null;try{raw=new Uint8Array(await file.arrayBuffer());key=await keyFileComponent(raw);const pass=$('unlock-recovery-password').value||null;const u=record?.external?await openStoredKdbxGeneric(record,pass,key):await openStoredKdbxWithRecoveryKey(record,key,pass);await acceptKdbxOpen(u);clearSecretInputs();enterApp();showToast('Cofre aberto com key file KeePass/KeePassXC.','success');}catch(e){$('unlock-error').textContent=e.message||String(e);}finally{raw&&wipe(raw);key&&wipe(key);$('unlock-recovery-key-file').value='';}
}

function enterApp(){setPublicScreen('app');$('app-vault-name').textContent=session.vault.name||'Meu Cofre';$('search').value='';$('add-entry').hidden=false;selectedGroupUuid='all';ensureVaultGroups();renderGroups();renderEntries();renderSecurity();renderSettings();navigate('vault');resetIdleTimer();if(isExternalKdbx())showToast('KDBX KeePassXC aberto em modo editável. Pastas, histórico, anexos e campos avançados são preservados.','success');}
function clearSessionMemory(){closeVeraCryptSession(true);if(session?.vaultKey)wipe(session.vaultKey);for(const c of session?.components||[])wipe(c);session=null;editingId=null;editingGroupUuid=null;selectedGroupUuid='all';clearTimeout(idleTimer);clearTimeout(backgroundTimer);clearTimeout(clipboardTimer);stopTotpTimer();clearSecretInputs();}
...
function ensureVaultGroups(){
  if(!session?.vault)return;const v=session.vault;let groups=Array.isArray(v.groups)?v.groups:[];let root=groups.find(g=>g.isRoot)||groups.find(g=>g.kdbxUuidBytes===v.rootGroupUuid);
  if(!root){const ru=v.rootGroupUuid||bytesToBase64(crypto.getRandomValues(new Uint8Array(16)));root={id:uuid(),kdbxUuidBytes:ru,parentUuid:null,name:v.name||'Meu Cofre',notes:'',iconId:'48',isExpanded:true,enableAutoType:'null',enableSearching:'null',defaultAutoTypeSequence:'',createdAt:v.createdAt||new Date().toISOString(),updatedAt:v.updatedAt||new Date().toISOString(),isRoot:true};groups=[root,...groups];}
  root.isRoot=true;root.parentUuid=null;v.rootGroupUuid=root.kdbxUuidBytes;v.groups=groups;const valid=new Set(groups.map(g=>g.kdbxUuidBytes));for(const g of groups)if(!g.isRoot&&(!g.parentUuid||!valid.has(g.parentUuid)))g.parentUuid=root.kdbxUuidBytes;for(const e of v.entries||[])if(!e.groupUuid||!valid.has(e.groupUuid))e.groupUuid=root.kdbxUuidBytes;
}
function rootGroup(){ensureVaultGroups();return session.vault.groups.find(g=>g.isRoot)||session.vault.groups[0];}
function groupByUuid(u){ensureVaultGroups();return session.vault.groups.find(g=>g.kdbxUuidBytes===u)||null;}
function childGroups(parent){return session.vault.groups.filter(g=>!g.isRoot&&g.parentUuid===parent).sort((a,b)=>String(a.name).localeCompare(String(b.name),'pt-BR'));}
function groupDepthMap(){const root=rootGroup(),out=new Map([[root.kdbxUuidBytes,0]]);const walk=(p,d)=>{for(const g of childGroups(p)){out.set(g.kdbxUuidBytes,d);walk(g.kdbxUuidBytes,d+1);}};walk(root.kdbxUuidBytes,1);return out;}
function groupDescendants(uuid){const set=new Set([uuid]),walk=u=>{for(const g of childGroups(u)){set.add(g.kdbxUuidBytes);walk(g.kdbxUuidBytes);}};walk(uuid);return set;}
function groupPath(uuid){const parts=[];let g=groupByUuid(uuid),guard=0;while(g&&guard++<64){parts.unshift(g.name||'Pasta');g=g.parentUuid?groupByUuid(g.parentUuid):null;}return parts.join(' / ');}
function groupEntryCount(uuid,directOnly=true){if(uuid==='all')return session.vault.entries.length;const allowed=directOnly?new Set([uuid]):groupDescendants(uuid);return session.vault.entries.filter(e=>allowed.has(e.groupUuid)).length;}
function selectGroup(uuid){selectedGroupUuid=uuid==='all'?'all':(groupByUuid(uuid)?.kdbxUuidBytes||rootGroup().kdbxUuidBytes);$('search').value='';renderGroups();renderEntries();}
function renderGroups(){if(!session)return;ensureVaultGroups();const tree=$('group-tree');tree.replaceChildren();const root=rootGroup();$('group-all-count').textContent=String(session.vault.entries.length);$('group-all').classList.toggle('active',selectedGroupUuid==='all');$('edit-current-group').hidden=selectedGroupUuid==='all'||selectedGroupUuid===root.kdbxUuidBytes;$('group-context').textContent=selectedGroupUuid==='all'?'Estrutura KeePassXC':groupPath(selectedGroupUuid);
  const make=(g,depth)=>{const row=document.createElement('button');row.type='button';row.className='group-node'+(selectedGroupUuid===g.kdbxUuidBytes?' active':'');row.style.setProperty('--depth',String(depth));row.addEventListener('click',()=>selectGroup(g.kdbxUuidBytes));const icon=document.createElement('span');icon.className='group-folder';icon.textContent='▸';const name=document.createElement('span');name.className='group-name';name.textContent=g.isRoot?(g.name||session.vault.name||'Raiz'):(g.name||'Pasta');const count=document.createElement('small');count.textContent=String(groupEntryCount(g.kdbxUuidBytes,true));row.append(icon,name,count);tree.append(row);for(const c of childGroups(g.kdbxUuidBytes))make(c,depth+1);};make(root,0);
}
...
function newGroupUuid(){return bytesToBase64(crypto.getRandomValues(new Uint8Array(16)));}
function openGroupModal(uuid=null){if(!session)return;ensureVaultGroups();editingGroupUuid=uuid;const g=uuid?groupByUuid(uuid):null;if(g?.isRoot)return;$('group-modal-title').textContent=g?'Editar pasta':'Nova pasta';$('group-name').value=g?.name||'';const excluded=new Set();if(g)for(const u of groupDescendants(g.kdbxUuidBytes))excluded.add(u);populateGroupSelect($('group-parent'),g?.parentUuid||(selectedGroupUuid==='all'?rootGroup().kdbxUuidBytes:selectedGroupUuid),{exclude:excluded});$('group-delete').hidden=!g;$('group-modal').hidden=false;setTimeout(()=>$('group-name').focus(),60);}
function closeGroupModal(){if(!$('group-modal'))return;$('group-modal').hidden=true;editingGroupUuid=null;$('group-form').reset();}
async function saveGroupFromForm(ev){ev.preventDefault();if(!session)return;const name=$('group-name').value.trim();if(!name)return showToast('Informe o nome da pasta.','error');ensureVaultGroups();const now=new Date().toISOString();let g=editingGroupUuid?groupByUuid(editingGroupUuid):null;if(g){g.name=name;g.parentUuid=$('group-parent').value||rootGroup().kdbxUuidBytes;g.updatedAt=now;}else{g={id:uuid(),kdbxUuidBytes:newGroupUuid(),parentUuid:$('group-parent').value||rootGroup().kdbxUuidBytes,name,notes:'',iconId:'48',isExpanded:true,enableAutoType:'null',enableSearching:'null',defaultAutoTypeSequence:'',createdAt:now,updatedAt:now,isRoot:false};session.vault.groups.push(g);selectedGroupUuid=g.kdbxUuidBytes;}try{await persistVault();closeGroupModal();renderGroups();renderEntries();showToast('Estrutura de pastas salva no KDBX.','success');}catch(e){showToast(e.message||String(e),'error');}}
async function deleteCurrentGroup(){if(!session||!editingGroupUuid)return;const g=groupByUuid(editingGroupUuid);if(!g||g.isRoot)return;const children=childGroups(g.kdbxUuidBytes),count=groupEntryCount(g.kdbxUuidBytes,true);if(children.length||count){showToast('Mova ou exclua os itens e subpastas antes de excluir esta pasta.','error');return;}if(!confirm(`Excluir a pasta “${g.name}”?`))return;session.vault.groups=session.vault.groups.filter(x=>x.kdbxUuidBytes!==g.kdbxUuidBytes);selectedGroupUuid=g.parentUuid||rootGroup().kdbxUuidBytes;try{await persistVault();closeGroupModal();renderGroups();renderEntries();showToast('Pasta excluída do KDBX.');}catch(e){showToast(e.message||String(e),'error');}}
...
async function renderSecurity(){if(!session)return;const legacy=isLegacySession();$('legacy-migration-section').hidden=!legacy;$('kdbx-summary').replaceChildren();$('auth-slots').replaceChildren();$('recovery-key-section').hidden=true;$('change-password-section').hidden=true;
  ...
  else {renderKdbxSummary();renderKdbxSlots();if(!isExternalKdbx()){const mode=session.publicMeta.mode;$('recovery-key-section').hidden=mode===PROTECTION_MODES.PASSWORD;$('change-password-section').hidden=mode===PROTECTION_MODES.YUBIKEY;}}
  await renderDiagnostics();$('backup-status').textContent=isExternalKdbx()?'KDBX KeePassXC editável: ao salvar, o Meu Cofre preserva grupos, anexos, campos extras, ícones e histórico existentes; alterações nos campos exibidos são regravadas no próprio KDBX.':(isKdbxSession()?'O arquivo .kdbx contém o cofre cifrado e autenticado. Exporte após mudanças importantes.':'Migre primeiro para KDBX 4.1.');
}
function renderKdbxSummary(){const info=session.kdbxInfo||{};let kdf=info.kdf||'AES-KDF';if(kdf==='AES-KDF')kdf+=info.rounds?` · ${Number(info.rounds).toLocaleString('pt-BR')} rodadas`:'';else if(/^Argon2/.test(kdf))kdf+=` · ${Number(info.iterations||0).toLocaleString('pt-BR')} iterações · ${Math.round(Number(info.memoryBytes||0)/1048576)} MiB · p=${info.parallelism||1}`;let protection;if(isExternalKdbx()){const p=session.publicMeta.requiresPassword,k=session.publicMeta.requiresKeyFile;protection=p&&k?'Senha + key file':p?'Senha':k?'Key file':'Credenciais KeePassXC';}else protection=modeLabel(session.publicMeta.mode);const data=[['Formato',`KDBX ${info.version||'4.1'}`],['Origem',isExternalKdbx()?'KeePassXC/KeePass importado · editável · estrutura avançada preservada':'Meu Cofre'],['Proteção',protection],['Cifrador',info.cipher||'AES-256-CBC'],['KDF',kdf],['Compressão',info.compression||'Nenhuma']];for(const [a,b] of data){const r=document.createElement('div');r.className='row';const m=document.createElement('div');m.className='row-main';const t=document.createElement('div');t.className='row-title';t.textContent=a;const sub=document.createElement('div');sub.className='row-sub';sub.textContent=b;m.append(t,sub);r.append(m);$('kdbx-summary').append(r);}}
function renderKdbxSlots(){const slots=session.publicMeta.slots||[];if(!slots.length){const d=document.createElement('div');d.className='empty';d.textContent=isExternalKdbx()?'Nenhuma YubiKey FIDO2 alternativa cadastrada. As credenciais originais do KeePassXC continuam válidas.':(session.publicMeta.mode===PROTECTION_MODES.PASSWORD?'Nenhuma YubiKey cadastrada.':'Nenhuma chave cadastrada — use a chave de recuperação antes de bloquear.');$('auth-slots').append(d);return;}for(const slot of slots){const r=document.createElement('div');r.className='row';const m=document.createElement('div');m.className='row-main';const t=document.createElement('div');t.className='row-title';t.textContent=slot.label||(slot.kind==='platform'?'Este iPhone':'YubiKey');const sub=document.createElement('div');sub.className='row-sub';const role=slot.role==='component-bundle'?'credenciais KeePassXC completas':slot.role==='keyfile'?'componente de chave':'componente de senha';sub.textContent=`WebAuthn PRF/HKDF · ${formatDateTime(slot.createdAt)} · ${role}`;m.append(t,sub);const pill=document.createElement('span');pill.className='pill ok';pill.textContent=slot.kind==='platform'?'Passkey':'YubiKey';const remove=document.createElement('button');remove.className='btn danger small';remove.textContent='Remover';remove.addEventListener('click',()=>removeKdbxSlot(slot.id));r.append(m,pill,remove);$('auth-slots').append(r);}}
...
async function saveSettings(){if(!session)return;session.vault.settings={...session.vault.settings,idleLockMinutes:clampInt($('setting-idle').value,1,120,5),backgroundLockSeconds:clampInt($('setting-background').value,0,300,0),clipboardClearSeconds:clampInt($('setting-clipboard').value,0,300,20)};if(isExternalKdbx()){record={...record,sidecarPrefs:{...session.vault.settings},updatedAt:new Date().toISOString()};await putVaultRecord(record);}else await persistVault();showToast('Configurações salvas.','success');}
