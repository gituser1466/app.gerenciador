...
function enterApp(){setPublicScreen('app');$('app-vault-name').textContent=session.vault.name||'Meu Cofre';$('search').value='';$('add-entry').hidden=false;renderEntries();renderSecurity();renderSettings();navigate('vault');resetIdleTimer();if(isExternalKdbx())showToast('KDBX KeePassXC aberto em modo editável. Campos avançados não exibidos são preservados na gravação.','success');}
